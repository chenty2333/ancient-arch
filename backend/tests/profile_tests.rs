...
async fn test_profile_complex_flow() {
    ...
    for i in 1..=2 {
        client.post(&format!("{}/api/posts", address))
    ...
    let posts_a: Vec<serde_json::Value> = client.get(&format!("{}/api/posts", address))
        .send().await.unwrap().json().await.unwrap();
    
    let post_a1_id = posts_a.iter().find(|p| p["title"] == "A Post 1").unwrap()["id"].as_i64().unwrap();
    let post_a2_id = posts_a.iter().find(|p| p["title"] == "A Post 2").unwrap()["id"].as_i64().unwrap();

    // B likes A1
    client.post(&format!("{}/api/posts/{}/like", address, post_a1_id))
        .header("Authorization", format!("Bearer {}", token_b))
        .send().await.unwrap();
    
    // B favorites A2
    client.post(&format!("{}/api/posts/{}/favorite", address, post_a2_id))
        .header("Authorization", format!("Bearer {}", token_b))
        .send().await.unwrap();

    // 4. Test /api/profile/me for User A
    let me_a = client.get(&format!("{}/api/profile/me", address))
        .header("Authorization", format!("Bearer {}", token_a))
        .send().await.unwrap().json::<serde_json::Value>().await.unwrap();
    ...
    let favs_b = client.get(&format!("{}/api/profile/favorites", address))
        .header("Authorization", format!("Bearer {}", token_b))
        .send().await.unwrap().json::<Vec<serde_json::Value>>().await.unwrap();
    ...
    let my_posts_a = client.get(&format!("{}/api/profile/posts", address))
        .header("Authorization", format!("Bearer {}", token_a))
        .send().await.unwrap().json::<Vec<serde_json::Value>>().await.unwrap();
    
    let a1_status = my_posts_a.iter().find(|p| p["id"].as_i64() == Some(post_a1_id)).unwrap();
    ...
    client.post(&format!("{}/api/contributions", address))
        .header("Authorization", format!("Bearer {}", token_a))
        .json(&contrib_payload)
        .send().await.unwrap();

    let my_contribs_a = client.get(&format!("{}/api/profile/contributions", address))
        .header("Authorization", format!("Bearer {}", token_a))
        .send().await.unwrap().json::<Vec<serde_json::Value>>().await.unwrap();
